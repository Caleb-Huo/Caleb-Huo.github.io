...
#' ===
#' 
#' - if there exists other columns with identical names from two dataFrames
#' 
data1 = pd.DataFrame({"keyA": list("bccaab"), "keyB": list("AABBCC"), "value1": range(6)})
data2 = pd.DataFrame({"keyA": list("abc"), "keyB": list("CBA"), "value2": range(3)})
...
#' 
#' - Part 1
#'   - unstack df_right such that only Key1 is the index
#'   - merge df_left and unstacked df_right
#' - Part 2
#'   - unstack df_right such that only Key2 is the index
#'   - merge df_left and unstacked df_right
#'   
#' ---
#' 
...
#' ===
#' 
#' - three ways to calculate mean value
#' 
...
#' ===
#' 
#' - For each subject of the sleep data, calculate the 
#'   - mean 
#'   - std 
#'   - max
#'   - second largest reaction time
#'   - maxDiff
#' 
sleepdata.head()
#' 

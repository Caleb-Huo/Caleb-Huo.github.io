...
#'     - $\mu = E(y|X) = \frac{\exp (X^\top \beta)}{1 + X^\top \beta}$
#' 
#' 
...
#' ===
#' 
#' - Generalized liner model (GLM) allows the user to select a distribution from the **exponential family**, which gives much greater flexibility in specifying the varaince structure of the outcome variable.
#' - The exponential family comprises a set of flexible distribution ranging both continuous and discrete random varaibles.
#' - Many of the probability distributions that we commonly used are specific cases of this family.
#' 
#' 
...
#' 
#' - Canonical parameter $\theta = \log(\frac{1}{1 - \mu})$
#' 
#' 
...
#' - sign of $\beta_1$ indicates the direction of the curve:
#'     - if $\beta_1 > 0$, the curve is an increasing function ($x$ up, $\mu$ up)
#'     - if $\beta_1 < 0$, the curve is an decreasing function ($x$ up, $\mu$ down)
#' 
## ------------------------------------------------------------------------
...
#' $$
#' 
#' - $\beta_1$ is log odds ratio of $y=1$ comparing $x=1$ to $x=0$
#' - $\exp(\beta_1)$ is odds ratio of $y=1$ comparing $x=1$ to $x=0$
#' 
#' 
...
#' $$
#' 
#' - $\beta_1$ is log odds ratio of $y=1$ comparing $x=1$ to $x=0$ after adjusting for other covariates
#' - $\exp(\beta_1)$ is odds ratio of $y=1$ comparing $x=1$ to $x=0$ after adjusting for other covariates
#' 
#' 
...
#'     
#' - Will learn in lecture optimization.
#' 
#' 
...
#' - The coefficient $\beta_j$ can be interpreted as: the increase in log-odds of falling into category $j$ versus category 1 (the reference category) resulting from a one-unit increase in covariate $X$ holding the other covariates constant.
#' 
#' 
#' Multinomial logistic Regression example
#' ===
#' 
#' - The modeling of program choices made by high school students can be done using Multinomial logit. 
#' - The program choices are general program, vocational program and academic program. 
#' - Their choice can be modeled using their writing score and their social economic status.
#' 
## ------------------------------------------------------------------------
...
#' 
#' $$log (\frac{P(Y\le j | X)}{P(Y > j | X)}) = log (\frac{P(Y\le j | X)}{1 - P(Y \le j | X)}) = \alpha_j - \beta^T X$$
#' For all $j = 1, \ldots, J - 1$.
#' 
#' - The sign of the coefficient $\beta$ indicates that
#'     - If $\beta > 0$: values of $X$ increase $\rightarrow$ cumulative probablities are lower $\rightarrow$ it is more likely to observe higher values of $Y$.
#'     - If $\beta < 0$: values of $X$ increase $\rightarrow$ cumulative probablities are higher $\rightarrow$ it is more likely to observe lower values of $Y$.
#' 
...
#' - The most common link used is the log link.
...
#' - The problem with this formulation is that it can yeiled values of $\mu < 0$
#' 
#' 

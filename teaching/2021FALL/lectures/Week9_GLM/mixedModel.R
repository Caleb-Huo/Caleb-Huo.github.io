...
#' - Under the same scenario $j$, people should also react similarly, not totally independent.
#'     - $frequency_{1jk}, frequency_{2jk}, \ldots, frequency_{Ijk}$ should be correlated
#' 
#' data independence
#' ===
#' 
...
#'       - The “group” effect $\alpha_i$ for subject $i$ is best thought of as random: $\alpha_i \sim N(0, \sigma_0^2)$
#' 
...
#'    - Random effect: error term is considered to be correlated.
...
#' - need to use REML = FALSE, otherwise, anova will re-fit the reml model.
#' 
## -----------------------------------------------------------------------------------------------------------
rs_subj_ml = lmer(frequency ~ attitude + (1 | subject), REML = FALSE, data = d)
...
#' ===
#' 
#' - Examine the relationship of Reaction time with respect to Days

...
#' $$Av = \lambda v$$
#' where $\lambda \in \mathbb{R}$ is the eigenvalue corresponding to $v$.
#' Since if $v$ is eigenvector, $bv$ ($b\in \mathbb{R}$) is also eigenvector, we restrict $\|v\|_2 = 1$
#' 
#' - Let $A \in \mathbb{R}^{N \times N}$ be a square  matrix with N linearly independent eigenvectors, $v_i (i = 1, \ldots, N)$.
...
#'   - If $m > n$
#'     - $U \in \mathbb{R}^{m \times n}$ such that $U^\top U = I_m$.
